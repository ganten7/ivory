...
class ChordDetector:
    ...
    def detect_chord(self, active_notes: Set[int], lowest_note: Optional[int] = None) -> Optional[str]:
        ...
        if best_match and best_root_pc is not None and lowest_pc != best_root_pc:
            ...
            if skip_slash_for_dominant_bass:
        ...
        if best_match and best_root_pc is not None:
            ...
            if has_major_third and has_natural_11 and is_major_quality:
                # Check if the natural 11 is ONLY in the bass octave
                p4_pc = (best_root_pc + 5) % 12  # The P4 note
                lowest_note = min(active_notes)
                lowest_octave_top = lowest_note + 12

                # Check if P4 appears above the bass octave
                p4_above_bass = False
                for note in active_notes:
                    if note % 12 == p4_pc and note > lowest_octave_top:
                        p4_above_bass = True
                        break
                ...
                if p4_above_bass:
                    ...
                    return None
                # else: Natural 11 only in bass - allow it
    ...
    def _match_chord_pattern(self, intervals: List[int], root_pc: int,
        ...
        for chord_type, pattern in CHORD_PATTERNS.items():
            ...
            if root_pc == lowest_pc and 0 in matched_intervals:
                ...
                if (chord_type.endswith('13') or chord_type.endswith('11') or chord_type.endswith('9') or
                    'minor13' in chord_type or 'minor11' in chord_type or 'minor9' in chord_type or
                    'major13' in chord_type or 'major11' in chord_type or 'major9' in chord_type or
                    'dominant13' in chord_type or 'dominant11' in chord_type or 'dominant9' in chord_type):
